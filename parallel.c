...
void parallelPhysicsEngine(){
   ...
   double timeIncrement = DELTATIME / (double)PHYSICSUPDATESPERFRAME; //calculated the time step before the loops since it makes no sense to compute it every time
   int idx;
   #pragma simd
   ...
   //adding a pragma omp for here will crash the program!
...
for(i = 0; i < SATELLITE_COUNT; ++i) {
    doublevector positionToBlackHole = { .x = tmpPosition[i].x - tmpMousePosX, .y = tmpPosition[i].y - tmpMousePosY };
    ...
    // Gravity force with cumulative updates
    ...
    tmpVelocity[i].x -= accumulation * normalizedDirection.x * timeIncrement;
    tmpVelocity[i].y -= accumulation * normalizedDirection.y * timeIncrement;

    // Update position based on cumulative velocity
    tmpPosition[i].x += tmpVelocity[i].x * timeIncrement;
    tmpPosition[i].y += tmpVelocity[i].y * timeIncrement;
}
   }
...
void parallelGraphicsEngine() {
    ...
    for(int i = 0; i < SIZE; ++i) {
        ...
        // Calculate distance to black hole using squared distances
        floatvector positionToBlackHole = {.x = pixel.x - tmpMousePosX, .y = pixel.y - tmpMousePosY};
        ...
        if (distToBlackHoleSquared < BLACK_HOLE_RADIUS * BLACK_HOLE_RADIUS) {
            ...
            pixels[i].green = 0;
            pixels[i].blue = 0;
            continue; // Skip to next pixel, black hole drawing done
        }

        // Initialize render color
        color_f32 renderColor = {.red = 0.f, .green = 0.f, .blue = 0.f};

        // Find closest satellite and calculate weights
        float shortestDistanceSquared = INFINITY;
        float weights = 0.f;
        int hitsSatellite = 0;

        // Iterate over satellites to find the closest one and check hits
        for (int j = 0; j < SATELLITE_COUNT; ++j) {
            ...
            if (distanceSquared < SATELLITE_RADIUS * SATELLITE_RADIUS) {
                // The pixel is inside the satellite, render it white
                renderColor.red = 1.0f;
                renderColor.green = 1.0f;
                renderColor.blue = 1.0f;
                hitsSatellite = 1;
                break; // Exit early since we hit a satellite
            } else {
                // Calculate the weight based on inverse distance squared
                float weight = 1.0f / (distanceSquared * distanceSquared);
                weights += weight;

                // Track the closest satellite for color rendering
        ...
        // If no satellite was directly hit, adjust color based on distance weights
        if (!hitsSatellite) {
            for (int k = 0; k < SATELLITE_COUNT; ++k) {
                ...
                renderColor.red += (satellites[k].identifier.red * weight / weights) * 3.0f;
                renderColor.green += (satellites[k].identifier.green * weight / weights) * 3.0f;
                renderColor.blue += (satellites[k].identifier.blue * weight / weights) * 3.0f;
            }
        }

        // Convert the color to 8-bit format and store it
